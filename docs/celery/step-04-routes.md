...
Клиент сам решает, как часто спрашивать статус. В этом гайде — руками через `curl`. Сокет, который сам присылает растущий `book_count`, — отдельный урок: [../websockets/README.md](../websockets/README.md). Его не подключайте, пока опрос через `GET` не работает.
