# Шаг 6 — Финальный прогон и сравнение

**Предыдущий:** [step-05-base.md](step-05-base.md) · **К карте:** [guide.md](guide.md)

## Задача

Прогнать API на FastAPI целиком: сиды, CRUD + PATCH, Swagger, клиент задачи 1 без изменений.
...
| ☐ | `/docs` строится из типов, без YAML |
| ☐ | `task_1_client` по-прежнему на aiohttp |
| ☐ | Роуты не импортируют `sqlalchemy.select` |
| ☐ | Общий SQL — в `BaseRepository`, репозитории не импортируют FastAPI |
...
- Пагинация и фильтры — отдельные методы потомка, не универсальный поисковый движок в базе.
